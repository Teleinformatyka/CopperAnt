...
public class DeviceLoggingModuleFacade {
	private final Logger log = Logger.getLogger(DeviceLoggingModuleFacade.class);
	...
	public Logger getDeviceLogger(WithControl deviceWithControl) {
		...
		if (deviceControl!=null) {
			...
			if (logTabPane.hasTab(deviceControlId)) {
				TextArea logsTextArea = logTabPane.getTabTextArea(deviceControlId);
				...
				sucessfullyAssignedDeviceToControl.put(deviceId, deviceControlId);	//hack
			} else {
				log.error("Tab with id: '" + deviceControlId + "' hasn't been created! Unable to log from: " + deviceId);
			}
		} else {
	...
	public void updateDeviceLoggerWithControl(WithControl deviceWithControl) {	//hack
		...
		if (deviceControl!=null) {
			...
			String oldControl = sucessfullyAssignedDeviceToControl.get(deviceControl);
		...
		//todo przemyslec przypadek gdy juz byl przypisany control, jest tab, a wolane update()
	}
